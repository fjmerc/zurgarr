...
| `TAUTULLI_URL` | Tautulli base URL (e.g. `http://tautulli:8181`). Enables watch-history correlation: the Wanted recovery pass reads play history so its per-scan budgets favor titles people actually watch. Read-only — zurgarr never writes to Tautulli. The API key is sent as a query parameter (Tautulli has no header auth); zurgarr strips query strings from logged URLs, so the key never reaches logs | — |
| `TAUTULLI_API_KEY` | Tautulli API key (Settings → Web Interface → API) | — |
| `WANTED_DEPRIORITIZE_UNPLAYED` | With Tautulli configured, Wanted-recovery targets whose title has no recorded plays sort to the back of the queue (stable — the existing order is preserved within the played/unplayed bands). Ordering only: nothing is skipped, no budgets change, and an unreachable Tautulli degrades to the normal order. Inert without Tautulli | `true` |
| `TAUTULLI_HISTORY_DAYS` | Watch-history lookback: a play within this many days counts as "played" (partial watches included) | `180` |
